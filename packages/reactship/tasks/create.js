...
const createProjectAsync = require('@easyship/react-template/create-project');

module.exports = () => {
    logger.info('Configuring new project creation...');

    const template = argv.uargs[0];
    const projectName = argv.uargs[1];
    ...
    define(
        'create:new-project',
        async done => {
            await createProjectAsync(template, projectPath);
            done();
        }
    );
